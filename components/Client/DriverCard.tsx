...
const DriverCard = ({fname,lname,team} : prop) => {
  ...
  const [acColor,setAcColor] = useState(() => {
    for(let team_obj of t.teams){
      if(team_obj.name == team) return team_obj.accolor
    }
    return "error"
  })
  ...
  const textStyle : CSSProperties = {
    backgroundImage: "linear-gradient(to bottom, transparent"+", "+acColor+" )",
    opacity: 1
  }

  return (
    <div className="relative shrink-0 aspect-[0.6666] flex flex-col w-[181px] items-center rounded-[8px]" style={{backgroundColor:bgColor}}>
        ...
        <div className="absolute z-1 bottom-[0px] w-full pt-[8px] rounded-[8px]" style={textStyle}>
          <div className="flex flex-col items-center pt-[16px] pb-[16px] rounded-[8px] w-full">
